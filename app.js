...
const processOrders = async () => {
    ...
    const myAsks = openOrders.data.asks.filter(ask => ask[3]==askPriceCAT && ask[4]==askPriceXCH);
    const myBids = openOrders.data.bids.filter(bid => bid[3]==bidPriceCAT && bid[4]==bidPriceXCH);
    ...
    if (myAsks.length < askMinOrders) {
        ...
        offer = {
            1:(askPriceXCH * xchMultiplier), // 1 always means XCH
            2:-(askPriceCAT * catMultiplier) // 2 is the first CAT in your wallet. You can get the index of your CAT
        };        
    }
